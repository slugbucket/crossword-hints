...
def crossword_setters_index(page):
    ...
    return render_template('crossword-setters/index.html',
                           setters=rs.dicts(),
                           pagination=Pagination(page, application.config['PER_PAGE'], count),
                           r=request, current_user=current_user)
...
def crossword_setters_new():
    if request.method == "GET":
        ...
        return render_template('crossword-setters/new.html', setter=setter, s_types=get_setter_types(), r=request, sbmt='Save new crossword setter')
    (rc, fdata) = sanitize_input(request.form)
    if not rc == "":
        flash(rc)
        return(render_template('crossword-setters/new.html', setter=fdata, s_types=get_setter_types(), r=request, sbmt=request.form['submit']))
...
def crossword_setters_edit(id):
    if request.method == "GET":
        try:
            ...
            return(redirect('/crossword-setters'))
        rs = crossword_setters.get(crossword_setters.rowid == id)
        return render_template('crossword-setters/edit.html', setter=rs, s_types=get_setter_types(), r=request, sbmt='Update crossword setter')
    (rc, fdata) = sanitize_input(request.form)
    if not rc == "":
        flash(rc)
        return(render_template('crossword-setters/edit.html', setter=fdata, r=request, sbmt=request.form['submit']))
